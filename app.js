...
const STORAGE_KEY = 'pei_projetista_integrado_v4_2_trios';
...
function initialState(){
  return {
    schema:'PEI-PROJETISTA-INTEGRADO-4.2-TRIOS',
    ...
    auxiliaries:[],
    qgbt:{ib:'',breaker:'',icu:'',bus:'',cable:'',dps:'',neutral:'',pe:'',selectivity:'',notes:''},
    ccm:{bus:'',icc:'',mainBreaker:'',ip:'',form:'',reserve:''},
    substation:{trafo:'',z:'',ip:'',is:'',icc:'',btBreaker:'',tc:'',tp:'',mtProtection:'',arrester:'',groundCable:'',rods:'',metering:'',notes:''},
    lighting:lightingBase.map(x=>({...x,flux:'',uf:'',mf:'',calc:0,adopted:'',watt:'',totalW:0,model:''})),
...
function motorPowerCandidates(profile,voltage){
  ...
  const medium=[7.5,11,15,18.5,22,30];
  const high=[18.5,22,30,37,45,55,75];
...
function generateFromCode(code){
  ...
  const reserve=pick(r,[15,20,20,25]);
  ...
  state.lighting=lightingBase.map(x=>({...x,area:+(x.area*pick(r,[.95,1,1.05])).toFixed(1),flux:'',uf:'',mf:'',calc:0,adopted:'',watt:'',totalW:0,model:''}));
  calcCache={};
  return state;
}
...
function calcDemand(){
  ...
  const suggested=TRAFO_STD.find(x=>x>=req)||Math.ceil(req/100)*100;
  return{installed,pd,qd,sd,pf,req,suggested};
}
...
function setupCalculators(){
  ...
  $('ctApply').addEventListener('click',()=>{const c=calcCache.ct;if(!c?.In)return alert('Calcule primeiro.');state.substation.trafo=c.s;state.substation.z=c.z;state.substation.is=+c.In.toFixed(2);state.substation.icc=+(c.icc/1000).toFixed(2);state.qgbt.icu=Math.ceil(c.icc/1000);syncFormFromState();saveState();refreshComputedUI();});
...
function renderQgbtSummary(){
  ...
  $('qgbtSummary').innerHTML=[['Demanda aparente',`${fmt(d.sd,1)} kVA`],['Corrente equivalente',`${fmt(ib,1)} A`],['Icc aproximada SE',`${fmt(t.iccKa,1)} kA`],['Tensão do QGBT',`${state.meta.voltage} V`]].map(([a,b])=>`<div class="summary-card"><small>${a}</small><strong>${b}</strong></div>`).join('');
}
function renderSubstationSummary(){
  const d=calcDemand(),t=calcTrafo(state.substation.trafo||d.suggested);
  $('substationSummary').innerHTML=[['Pd',`${fmt(d.pd,1)} kW`],['Sd',`${fmt(d.sd,1)} kVA`],['S requerida c/ reserva',`${fmt(d.req,1)} kVA`],['Transformador sugerido',`${fmt(d.suggested,1)} kVA`]].map(([a,b])=>`<div class="summary-card"><small>${a}</small><strong>${b}</strong></div>`).join('');
}
...
function renderLighting(){
  ...
  body.innerHTML=state.lighting.map((r,i)=>`<tr><td class="cell-tag">${r.name}</td><td><input type="number" step=".1" data-light="${i}" data-key="area" value="${r.area}"></td><td><input type="number" data-light="${i}" data-key="lux" value="${r.lux}"></td><td><input type="number" data-light="${i}" data-key="flux" value="${r.flux}"></td><td><input type="number" step=".01" data-light="${i}" data-key="uf" value="${r.uf}"></td><td><input type="number" step=".01" data-light="${i}" data-key="mf" value="${r.mf}"></td><td id="lcalc${i}">${fmt(r.calc,2)}</td><td><input type="number" data-light="${i}" data-key="adopted" value="${r.adopted}"></td><td><input type="number" data-light="${i}" data-key="watt" value="${r.watt}"></td><td id="ltotal${i}">${fmt(r.totalW,0)}</td><td><input data-light="${i}" data-key="model" value="${escapeHtml(r.model)}"></td></tr>`).join('');
...
function suggestedMaterials(){
  ...
  if(state.substation.trafo)add('TR-01',`Transformador trifásico ${state.substation.trafo} kVA, ${fmt(state.meta.primary/1000,1)} kV / ${state.meta.voltage} V`,1,'un','trafo');
  add('QGBT-01',`Quadro Geral de Baixa Tensão — barramento ${state.qgbt.bus||'___'} A`,1,'un','qgbt');
  add('CCM-01',`Centro de Controle de Motores — barramento ${state.ccm.bus||'___'} A`,1,'un','ccm');
  ...
  state.lighting.forEach((r,i)=>{if(num(r.adopted)>0)add(`LUM-${i+1}`,`Luminária ${r.name} — ${r.model||`${r.watt||'___'} W`}`,num(r.adopted),'un',`lum-${i}`);});
...
function generateMemorialText(force=true){
  ...
  const generated={
    ...
    substation:`A subestação foi desenvolvida para alimentação em ${fmt(state.meta.primary/1000,1)} kV e transformação para ${state.meta.voltage} V. Transformador adotado: ${state.substation.trafo||'[PREENCHER]'} kVA, impedância ${state.substation.z||state.meta.trafoZ}%. A equipe deverá manter coerência entre demanda, medição, proteção MT, correntes, aterramento e os critérios aplicáveis ao projeto.`,
    qgbt:`O QGBT recebe a alimentação do transformador e distribui os circuitos da instalação. Foram adotados disjuntor geral ${state.qgbt.breaker||'[PREENCHER]'} A, barramento ${state.qgbt.bus||'[PREENCHER]'} A e capacidade de interrupção ${state.qgbt.icu||'[PREENCHER]'} kA. Alimentador principal: ${state.qgbt.cable||'[PREENCHER]'} mm².`,
    ccm:`O CCM reúne as saídas dos motores M01 a M10. Os métodos selecionados pela equipe são: ${methods}. Cada decisão de acionamento foi feita a partir da condição operacional individual e deve permanecer justificada na seção Engenharia de Acionamentos. Barramento do CCM: ${state.ccm.bus||'[PREENCHER]'} A; Icc considerada: ${state.ccm.icc||'[PREENCHER]'} kA.`,
...
function statusModel(){
  ...
  const qgbtDone=state.qgbt.breaker&&state.qgbt.icu&&state.qgbt.bus&&state.qgbt.cable,ccmDone=state.ccm.bus&&state.ccm.icc&&state.ccm.mainBreaker&&motorsDone,seDone=state.substation.trafo&&state.substation.ip&&state.substation.is&&state.substation.icc&&state.substation.mtProtection;
...
function exportSubmission(){
  ...
  alert('Arquivo de entrega gerado. Envie este JSON ao docente junto com o PDF final.');
}
...
function buildReport(){
  ...
  const report=$('printReport');report.innerHTML=`
    ...
    <section class="report-page"><h2>11. Pranchas do projeto</h2><h3>Planta geral</h3><img src="assets/planta_geral.webp" alt="Planta geral"><h3>Fluxo produtivo e elétrico</h3><img src="assets/planta_fluxo.webp" alt="Fluxo"></section>
